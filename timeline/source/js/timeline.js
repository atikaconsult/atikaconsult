...
if(typeof VMM != 'undefined' && typeof VMM.Timeline == 'undefined') {
	...
	VMM.Timeline = function(w, h) {
		...
		var build = function() {
			...
			================================================== */
			VMM.attachElement($timeline, "");
			VMM.appendElement($timeline, "<div class='container main row'><div class='feature large-4'><div class='slider'></div></div><div class='navigation'></div></div>");
		...
		var buildSlide = function(dd, d_date) {
			...
			if (type.of(d_date) == "date") {
				...
				if (d.headline != null && d.headline != "") {
					if (d.type == "tweets") {
						...
						c._text += VMM.createElement("div", d.headline, "start headline////////");
					} else {
						c._text += VMM.createElement("div", d.headline, "headline");
					}
				}
			...
			if (_valid) {
				...
				if (typeof d.figure != 'undefined' && d.figure) {
					c._media = VMM.createElement("div", d.figure, "media-container");
					c._media = VMM.createElement("div", c._media, "media-wrapper");
					c._media = VMM.createElement("div", c._media, "media");
				}
	...
	VMM.Timeline.TimeNav = function(parent, content_width, content_height) {
		...
		var buildMarkers = function() {
			...
			for(var i = 0; i < data.length; i++) {
				...
				/* ADD DATE AND TITLE
				================================================== */
				VMM.appendElement(_marker_content, "<h3>" + VMM.Util.unlinkify($('<p/>').text($(data[i].title).find('.a-slot-content:eq(0)').text()).html()) + "</h3><h4>" + data[i].date + "</h4>");
	...
	VMM.Timeline.DataObj = {
		...
		parseHTML: function(d) {
			...
			if (VMM.Element.find("#timeline > section", "time")[0]) {
				...
				_data_obj.timeline.figure = '';
				if (VMM.Element.find("#timeline > section", "figure").length) {
					_data_obj.timeline.figure = VMM.Element.find("#timeline > section", "figure").html();
			}
			...
			VMM.Element.each("#timeline > ul > li", function(i, elem){
				...
				if (VMM.Element.find(this, "time") != 0) {
					...
					_date.figure = '';
					if (VMM.Element.find(this, "figure").length) {
						_date.figure = VMM.Element.find(this, "figure").html();
				}
